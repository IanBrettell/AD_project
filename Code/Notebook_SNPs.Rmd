...
Then ran again without the genotype query: `./plink --file snp_data --map3 --no-fid --no-parents --logistic --beta --reference-allele ref_alleles.txt --noweb --covar cov.txt`
...
exdata$probe_id <- rownames(exdata)
exdata <- select(exdata, probe_id, everything())
```
...
# annotate
sig_eqtls <- left_join(sig_eqtls, snp_gns, by = c("snps" = "refsnp_id"))
```
