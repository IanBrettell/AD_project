...
I was using the wrong ID data previously - we will run the above again substituting the correct data.
...
dim(exdata)
head(exdata)
metadata <- read.delim("~/Dropbox/eQTL/Data/aibl-ids-6.0.0-201712010300.txt", sep = "\t", header = T) ## NOTE: this file isn't actually in the Dropbox - I just put it there on my local computer. Will need to be uploaded. 
dim(metadata)
head(metadata)
...
```{r}
hist(metadata2$Age, xlab = "Age", main = "Distribution of AIBL sample ages")
```
...
# make apoe4 a factor (for ease of plotting later)
meta2$apoe4 <- as.factor(meta2$apoe4)
```
...
exdata2 <- dplyr::left_join(meta2, exdata1, by = "AIBL.Id") %>% 
...
Run limma on the expression data, having excluded the outliers (AIBL.Ids **771**, **914** and **918**)

```{r}
exdata3 <- dplyr::filter(exdata2, AIBL.Id != "771" & AIBL.Id != "914" & AIBL.Id != "918")
```
...
Combine lists of significant genes for each contrast.
...
nrow(signif_all)
```
Therefore `r nrow(signif_all)` genes are expressed differentially across at least two contrasts
...
```
`r length(setdiff(rownames(exdata), pid_symb$probe_id))`/22,011 probes are not accounted for in the annotation data?
...
signif_PET_pos_v_neg <- dplyr::filter(results_PETposvneg, P.Value < 0.05)
...
## Get KEGG results for PET yes/no genes

Attach gene data to PET 
```{r}
library(tidyverse)
sig_PET_pos_v_neg <- left_join(signif_PET_pos_v_neg, pid_all, by = "probe_id")
sig_PET_pos_v_neg$probe_id <- as.character(sig_PET_pos_v_neg$probe_id)# make probe id column a character vector to make merging easier later
...
```{r write table of DE genes bewteen PET-pos and -neg}
...
```{r}
ped <- read.table(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/genotype_data.txt"), header = T)
...
meta <- read.table(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/key_metadata.txt"), header = T) # read in metadata
...
```{r}
pk_ass <- read.table(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/plink/plink.assoc"), na.strings = NA, header = T, fill = T)
pk_fsh <- read.table(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/plink/plink.assoc.fisher"), na.strings = NA, header = T, fill = T)
...
snp_gns <- read.delim(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/aibl_snp_genes.txt"), sep = "\t", header = T)
library(dplyr)
...
sig_pk <- dplyr::select(sig_pk, 
                 ...
                 strand, 
								 ensembl_id = "ensembl_gene_stable_id", 
                 hgnc_symbol, 
...
```{r}
probes <- read.delim(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/affy_probes_annotated.txt"), header = T, sep = "\t")
...
sig_pk <- dplyr::select(sig_pk, -ensembl_id.y, -gene_symbol) %>% 
	rename(ensembl_id = "ensembl_id.x")
...
```{r}
# read in table
snp_gns <- read.delim(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/aibl_snp_genes.txt"), header = T)
...
Then ran again without the genotype query: `./plink --file snp_data --map3 --no-fid --no-parents --logistic --beta --reference-allele ref_alleles.txt --noweb --adjust --covar cov.txt`
...
```{r}
intersect(sig_snps_log$SNP, sig_pk$SNP)
intersect(sig_snps_log$hgnc_symbol, sig_pk$hgnc_symbol)
```
...
Only one SNP has a corrected p-value of less than 0.7!
...
ids2 <- read.delim("~/Dropbox/eQTL/Data/AIBL_expression_set/AIBL_Gene_Expression_IDs_UpdtdDec2017.txt", sep = " ")
colnames(exdata) <- as.vector(ids2$x) # replace colnames with ids2
meta <- read.delim(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/key_metadata.txt"), header = T, sep = "\t")
```
...
pk_frq <- read.table(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/plink/allele_freq.assoc.fisher"), na.strings = NA, header = T, fill = T)
...
# number of unique SNP genes
length(unique(sig_eqtls$gene))

# annotate
sig_eqtl_snps <- left_join(sig_eqtls, snp_gns, by = c("snps" = "refsnp_id"))
sig_eqtl_gns <- left_join(sig_eqtls, pid_all, by = c("gene" = "probe_id"))
...
Attach proxy SNPs to list of significant eqtl aibl snps

```{r}
snp_prx <- dplyr::select(snap2, SNP, Proxy)
snp_prx <- snp_prx[snp_prx$SNP %in% sig_eqtls$snps, ]
unique(snp_prx$SNP)
length(unique(snp_prx$Proxy))
```
Just two of the 17 significant eQTL AIBL SNPs had proxy SNPs! But those two are in LD with 12 proxy SNPs.

Find the genes in which those proxy SNPs lie.

```{r}
snp_prx <- dplyr::left_join(snp_prx, proxy_genes_full, by = c("Proxy" = "refsnp_id"))
unique(snp_prx$hgnc_symbol)
length(unique(snp_prx$hgnc_symbol))
```
Those proxy SNPs lie within 6 genes (7 less the NA).

So how many genes in total could affect the expression levels of those 12 genes? Combine the list of genes in which both the AIBL and proxy SNPs lie

```{r}
sig_snps_full <- as.vector(unique(c(unique(sig_eqtl_snps$snps), unique(snp_prx$Proxy))))
sig_snps_full
length(sig_snps_full)

sig_eqtl_gns_full <- as.vector(c(unique(as.character(sig_eqtl_snps$hgnc_symbol)), unique(snp_prx$hgnc_symbol)))
sig_eqtl_gns_full <- na.omit(sig_eqtl_gns_full)
sig_eqtl_gns_full
```

#### Find intersection between eQTL genes and original limma results
```{r}
intersect_lim_eqtl <- na.omit(intersect(sig_PET_pos_v_neg$gene_symbol, sig_eqtl_gns$gene_symbol))
intersect_lim_eqtl
```


### Carry out the same eQTL analysis but independently with the PET pos and neg groups
...
*AD nature paper - blood-based biomarker that is 90% accurate for PET status. Went viral all around the world.

*Track down gene map for 249/530 genes found in clusters, and match to limma outputs

*What did JD do with the SVD? In a scree plot, the first component explains 70-80% of the variance. 

*Each of the values in ll isa list of 218 vectors, which is the first eigenvalue.

*We have 249 root genes within each cluster.

* The correlated gene is only signficiant because the correlation between local group 1 and local group 2 was signficantly different. Then we take an SVD of those genes that are significantly different. 

*Then we do that for all 249 genes, and we compare our predictions using the eigenvalues to our initial limma results, and we can identify the genes for which using the eigenvalue has improved the prediction of AB. 

* Now assess which eQTL differentially expressed genes in the AB-pos group are in the 530 diffrentially expressed limma genes, and which ones sit within the 249 genes in the clusters. And do they intercept with any of the PLINK significant SNPs? 

*Get the gene names for the top 20 limma genes

*FInd the genes that were acting in cis in the eQTL

*Start thinking about biolgoical relationships

*For presentation

-design, understanding, background, results. Talk to the picture or figure. 

*New indices: Inflammatory burden, and cardiovascular disease index

*Look up Rothman, epidemiologist - looks at RERI, attributable proportion, and Synergy Index. All measure whether the genetic and environmental effects are additive... or multiplicative? Paper: Anderson, 'Meausres of biologicla interaction'
...
Use JD's code to compare the Enriched Network Decomposition (END) p-values with the p-values of the limma results from the raw expression data.

We'll first compare the limma results using the END carried out on *all* the genes in each network, even though most genes in each network weren't found to be differentially expressed in the initial analysis.

Then we'll do the same for just the genes in the clusters that were found to be DE in the intial analysis, and see if it improves prediction.

## END for all genes in networks
...
keymeta <- read.table(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/key_metadata.txt"), header = T)
...
Create eigenvalues for the filtered clusters.
...
autoplot(pca6, data = ll2d4.6, colour = "PET", label = T, label.label = "AIBL.Id") +
    viridis::scale_colour_viridis(option = "viridis", discrete = T) +
    ggtitle("PCA with significant clusters")
```
...
```{r}
library(rjson)
filtered_clusters <- fromJSON(file = "~/Dropbox/eQTL/Results/cluster_data/gene_clusters_filtered.json")
filtered_clusters1 <- lapply(filtered_clusters, function(x){ # extract just the genes
  return(x$genes)
})
dingo_filtered_probes <- unique(unlist(lapply(filtered_clusters1, function(x){
  a <- as.character(x)
  return(a)
})))
...
# save RData file of filtered_clusters and dingo_filtered_probes
save(filtered_clusters, file = "~/Desktop/Uni Air/CSIRO/AD_project/Working/networks/filtered_clusters.RData")

save(dingo_filtered_probes, file = "~/Desktop/Uni Air/CSIRO/AD_project/Working/networks/dingo_filtered_probes.RData")
...
```
This is the same number of unique probes as in 'cluster_genexp', which means the latter had already been filtered. 
...
### Create function to create eigenvalues
...
```{r}
end_limma <- function(cluster_list){
  # create eigenvalues
  ll <- lapply(cluster_list, function (x) {
    xm <- as.matrix(log(x[,-1])) # creates a matrix, using log to standardise
    svd.decomp2 <- svd(xm) # applies singular value decomposition
...
end_filter <- function(list, lower, upper){
  ...
  dingo_clstrs_3 <- lapply(dingo_clstrs_2, function(x){
    which(x < quantile(x, lower) | x > quantile(x, upper))
  })
...
How many of the initial DE genes fell within networks?
...
end_clsts_de_gns_only <- end_limma(clst_tidy_3)

## write table
# write.table(end_clsts_de_gns_only, "~/Desktop/Uni Air/CSIRO/AD_project/Working/end_clsts_de_gns_only.txt", col.names = T, row.names = F, quote = F, sep = "\t")
...
```{r}
end_results_0.5 <- end_limma(half_percentile)
# join to table
...
```{r}
end_results_0.25 <- end_limma(quarter_percentile)
# join to table
...
```{r}
end_results_0.1 <- end_limma(point1_percentile)
# join to table
...
end_results_1 <- end_limma(one_percentile)
...
ggsave("C:/Users/bre227/Documents/R/AD_project/Working/comp_volc_plot_END_thresholds.jpeg", plot = plots_END, device = "jpeg", width = 100, height = 30, units = "cm")
```
...
ggsave("C:/Users/bre227/Documents/R/AD_project/Working/comp_volc_plot_root_genes.jpeg", plot = test, device = "jpeg", width = 100, height = 30, units = "cm")

test <- plot_grid(d, e, f, ncol = 3)
ggsave("C:/Users/bre227/Documents/R/AD_project/Working/comp_volc_plot_networks.jpeg", plot = test, device = "jpeg", width = 100, height = 30, units = "cm")
```
...
```{r}
# load table of original limma results
sig_orig_limma <- read.delim(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/sig_PET_pos_v_neg.txt"),
                                header = T)
head(sig_orig_limma)

# load table of SNPs (association)
sig_snps_assoc <-  read.delim(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/sig_PET_pos_v_neg_alleles_annotated.txt"),
                                header = T)
head(sig_snps_assoc)

# load table of SNPs (logistic regression)
sig_snps_log <- read.delim(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/sig_PET_pos_v_neg_snps.txt"),
                          header = T)
head(sig_snps_log)

# load table of eQTL SNPs and genes
sig_eqtl_snps <- read.delim(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/eqtl_snps.txt"))
head(sig_eqtl_snps)

sig_eqtl_gns <- read.delim(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/eqtl_gns.txt"),
                           header = T)
head(sig_eqtl_gns)

# load table of p-values for all clusters containing DE genes
end_clsts_de_gns_only <- read.delim(text = getURL("https://raw.githubusercontent.com/IanBrettell/AD_project/master/Working/end_clsts_de_gns_only.txt"),
                                    header = T)
head(end_clsts_de_gns_only)
```

### Acquire list of genes found in most differentially expressed networks using END

#### Make one list with names as probe IDs
```{r}
# get p-values for each gene in each cluster
clst_pvals <- lapply(clst_tidy_3, function(x){
  sapply(names(x)[-1], function(y){
    sig_orig_limma$P.Value[sig_orig_limma$probe_id %in% y]
  })
})

# remove all networks that weren't significant at the FDR level
sig_clst_pvals_prbs <- clst_pvals[names(clst_pvals) %in% end_clsts_de_gns_only$ID[end_clsts_de_gns_only$adj.P.Val < 0.05]]

# sort each list in order of p-value
sort_clst_pvals_prbs <- lapply(sig_clst_pvals_prbs, sort)

# sort the whole list by significance of the cluster from the END analysis
sort_all_clstrs_prbs <- sort_clst_pvals_prbs[match(sig_end_clsts_de_gns_only$ID, names(sort_clst_pvals_prbs))]
```

#### Make another with with names as gene names
```{r}
# replace probe IDs with gene names
clst_pvals_gns <- lapply(clst_pvals, function(x){
  out <- x
  names(out) <-  as.character(sig_orig_limma$gene_symbol[sig_orig_limma$probe_id %in% names(x)])
  return(out)
})

# remove all networks that weren't significant at the FDR level
sig_clst_pvals_gns <- clst_pvals_gns[names(clst_pvals_gns) %in% end_clsts_de_gns_only$ID[end_clsts_de_gns_only$adj.P.Val < 0.05]]

# sort each list in order of p-value
sort_clst_pvals_gns <- lapply(sig_clst_pvals_gns, sort)

# sort the whole list by significance of the cluster from the END analysis
sort_all_clstrs_gns <- sort_clst_pvals_gns[match(sig_end_clsts_de_gns_only$ID, names(sort_clst_pvals_gns))]
```

### Find which genes in the sorted significant cluster list intersect with genes that were found to be significant in the other analyses

#### Find which genes in the clusters were associated in the SNP assoc
```{r}
intersect_clstrs_snp_assoc <- lapply(sort_all_clstrs_gns, function(x){
  if (length(x[names(x) %in% sig_snps_assoc$hgnc_symbol]) > 0){
    return(x[names(x) %in% sig_snps_assoc$hgnc_symbol])
  } else {
    NULL
  }
})

# how many clusters had genes that were also associated in the SNP assoc?
length(which(lapply(intersect_clstrs_snp_assoc, is.null) == F))
```

#### Find which genes in the clusters were associated in the SNP log

```{r}
intersect_clstrs_snp_log <- lapply(sort_all_clstrs_gns, function(x){
  if (length(x[names(x) %in% sig_snps_log$hgnc_symbol]) > 0){
    return(x[names(x) %in% sig_snps_log$hgnc_symbol])
  } else {
    NULL
  }
})

# how many clusters had genes that were also associated in the SNP log?
length(which(lapply(intersect_clstrs_snp_log, is.null) == F))
```

#### Find which genes in the clusters were DE in the eQTL
```{r}
intersect_clstrs_eqtl <- lapply(sort_all_clstrs_gns, function(x){
  if (length(x[names(x) %in% sig_eqtl_gns$gene_symbol]) > 0){
    return(x[names(x) %in% sig_eqtl_gns$gene_symbol])
  } else {
    NULL
  }
})

# how many clusters had genes that were also DE in the eQTL?
length(which(lapply(intersect_clstrs_eqtl, is.null) == F))

# which genes are they?
aa <- lapply(intersect_clstrs_eqtl, function(x){
  names(x)[!is.null(x)]
})
int_gns <- setNames(unlist(aa, use.names = F), rep(names(aa), lengths(aa)))
rm(aa)
int_gns
...
### Create data frame for intersecting genes
```{r, warning = F}
int_gns_df <- data.frame(network_id = names(int_gns), intersecting_gene_eqtl = int_gns, row.names = NULL) %>%
  dplyr::left_join(dplyr::select(genemap, hsa, description = "name"), by = c("network_id" = "hsa")) %>% 
  dplyr::left_join(dplyr::select(sig_end_clsts_de_gns_only, ID, fdr_cluster = "adj.P.Val"), by = c("network_id" = "ID")) %>% 
  dplyr::left_join(dplyr::select(sig_eqtl_gns, gene_symbol, fdr_eqtl_gene = "FDR", assoc_snp = "snps"), by = c("intersecting_gene_eqtl" = "gene_symbol")) %>% 
  dplyr::select(network_id, description, fdr_cluster, intersecting_gene_eqtl, fdr_eqtl_gene, assoc_snp)

int_gns_df
```

## Boxplot for most differentiated networks

### Get network eigenvalue and expression data for root gene in network hsa05165 - human papillomavirus infection
```{r}
# pull root gene from network
rt <- names(sort_all_clstrs_prbs[['hsa05165']])[1]

# get expression data for root gene
hsa05165_eigen_plot <- cbind(exdata3[, 1:5], exdata3[rt])

# Get eigenvalues for networks
clst_eigens <- data.frame(end_eigen(clst_tidy_3))

# Combine data frames
clst_eigens$AIBL.Id <- as.integer(rownames(clst_eigens))
hsa05165_eigen_plot <- dplyr::left_join(hsa05165_eigen_plot,
                                        dplyr::select(clst_eigens, AIBL.Id, hsa05165),
                                        by = "AIBL.Id")
```
...
```{r}
# gene expression
ggplot(data = hsa05165_eigen_plot, aes(PET, `3156307`, fill = PET)) +
  geom_boxplot()

# network eigenvalues
ggplot(data = hsa05165_eigen_plot, aes(PET, hsa05165, fill = PET)) +
  geom_boxplot()
```

```{r}
neg_prb <- ggplot(dplyr::filter(hsa05165_eigen_plot, PET == "NEG"), aes(PET, `3156307`, fill = PET)) +
  geom_boxplot()
neg_eig <- ggplot(dplyr::filter(hsa05165_eigen_plot, PET == "NEG"), aes(PET, hsa05165, fill = PET)) +
  geom_boxplot()
pos_prb <- ggplot(dplyr::filter(hsa05165_eigen_plot, PET == "POS"), aes(PET, `3156307`, fill = PET)) +
  geom_boxplot()
pos_eig <- ggplot(dplyr::filter(hsa05165_eigen_plot, PET == "POS"), aes(PET, hsa05165, fill = PET)) +
  geom_boxplot()

require(gridExtra)
grid.arrange(neg_prb, neg_eig, pos_prb, pos_eig, ncol = 4)
```
...
rosmap_data <- read.delim("~/Dropbox/eQTL/Data/ROSMAP_Data/ROSMAP_arrayExpression_normalized.tsv")

# read in metadata
rosmap_meta <- read.csv("~/Dropbox/eQTL/Data/ROSMAP_Data/ROSMAP_clinical.csv")
rosmap_meta$projid <- as.character(rosmap_meta$projid) # to facilitate binding later
```

### Transpose and add metadata 
```{r}
# create vector of gene symbols
gene_symbols <- as.character(rosmap_data$Symbol)

# how many of the gene symbols are unique?
length(unique(gene_symbols))

# create vector of IDs
rosmap_ids <- as.character(colnames(rosmap_data)[4:length(colnames(rosmap_data))])
rosmap_ids <- gsub("ROS_|MAP_", "", rosmap_ids) # remove the 'ROS' and 'MAP' prefixes

# remove first 3 columns ("ProbeID", "TargetID", and "Symbol") to leave just expression data
rosmap_data1 <- as_tibble(rosmap_data[, -c(1:3)])
rosmap_data1 <- as_tibble(t(rosmap_data1)) # transpose to put samples in rows, genes in columns
head(rosmap_data1)

# add colnames (gene symbols)
colnames(rosmap_data1) <- gene_symbols
head(rosmap_data1)

# add column with sample IDs
rosmap_data1$sample_id <- rosmap_ids

# join with metadata and reorder
rosmap_data2 <- dplyr::left_join(rosmap_data1,
                                 dplyr::select(rosmap_meta, projid, study, age_at_visit_max, msex, ad_reagan),
                                 by = c("sample_id" = "projid")) %>% 
	na.omit
rosmap_data2 <- dplyr::select(rosmap_data2, sample_id, study, age_at_visit_max, msex, ad_reagan, everything())

# turn ad_reagan and msex into factors
rosmap_data2$ad_reagan <- factor(rosmap_data2$ad_reagan)
rosmap_data2$msex <- factor(rosmap_data2$msex)

# to ensure the labels in the plots reflect the sample IDs
# rosmap_data2 <- data.frame(rosmap_data2)
# rownames(rosmap_data2) <- rosmap_ids
```

### Run PCA to see whether there is any clustering
```{r}
# run PCA
pca_rosmap <- prcomp(rosmap_data2[6:ncol(rosmap_data2)], center = T, scale. = T)

library(ggfortify)

# colour by study ("Ros" or "MAP") 
autoplot(pca_rosmap, data = rosmap_data2, colour = "study", label = T) +
  viridis::scale_colour_viridis(option = "viridis", discrete = T)

# colour by ad_reagan
autoplot(pca_rosmap, data = rosmap_data2, colour = "ad_reagan", label = T) +
  viridis::scale_colour_viridis(option = "viridis", discrete = T)

# colour by sex 
autoplot(pca_rosmap, data = rosmap_data2, colour = "msex", label = T) +
  viridis::scale_colour_viridis(option = "viridis", discrete = T)
```
...
```{r}
rosmap_exprs <- as.matrix(t(rosmap_data2[,6:ncol(rosmap_data2)])) # leaving behind the first 5 metadata columns
rosmap_minimalSet <- ExpressionSet(assayData = rosmap_exprs)
rosmap_pData <- rosmap_data2[,1:5]
summary(rosmap_pData)
```

Create design matrix
```{r}
rosmap_Group <- rosmap_data2$ad_reagan
rosmap_design <- model.matrix(~0 + rosmap_Group)
colnames(rosmap_design) <- c("NEG", "POS")
```
...
Find differentially expressed genes
```{r}
rosmap_fit <- lmFit(rosmap_exprs, rosmap_design)
rosmap_cont.matrix <- makeContrasts(POSvsNEG = POS - NEG, levels = rosmap_design)
rosmap_fit <- contrasts.fit(rosmap_fit, rosmap_cont.matrix)
rosmap_fit2 <- eBayes(rosmap_fit)
topTable(rosmap_fit2, adjust = "BH")
```

### Create table of results and volcano plot
```{r}
rosmap_results_reagan_posvneg <- topTable(rosmap_fit2, n = Inf)
rosmap_results_reagan_posvneg$gene_symbol <- rownames(rosmap_results_reagan_posvneg)

# Create second function for volcano plots
vol_plot_2 <- function(limma_output, xvar = "logFC", yvar = "adj.P.Val", alpha = 0.05, title = "Volcano plot", FDR = c(T, F)){
  # load required packages
  require(tidyverse)
  require(ggrepel)
...
# Plot
vol_plot_2(rosmap_results_reagan_posvneg, xvar = "logFC", yvar = "adj.P.Val", alpha = 0.05, title = "Volcano plot", FDR = T)

```

Create table of significant results
```{r}
rosmap_signif_reagan_pos_v_neg <- dplyr::filter(rosmap_results_reagan_posvneg, adj.P.Val < 0.05)
dim(rosmap_signif_reagan_pos_v_neg)

# tidy gene symbols column
rosmap_signif_reagan_pos_v_neg$probe_id <- gsub(".x", "", rosmap_signif_reagan_pos_v_neg$probe_id)
```

## Get entrez IDs from Biomart

Create mart
```{r, results = "hide"}
# get vector of significant gene symbols
rosmap_sig_genes <- rosmap_signif_reagan_pos_v_neg$gene_symbol

library(biomaRt)
mart_genes <- useMart('ENSEMBL_MART_ENSEMBL', 'hsapiens_gene_ensembl')
rosmap_sig_genes_biomart_hits <- getBM(attributes = c("ensembl_gene_id", "chromosome_name", "start_position", "end_position", "strand", "hgnc_symbol", "entrezgene"),
                       filters = "hgnc_symbol",
                       values = rosmap_sig_genes,
                       mart = mart_genes)
```

Bind results to limma table of significant genes
```{r}
rosmap_signif_reagan_pos_v_neg1 <- dplyr::left_join(rosmap_signif_reagan_pos_v_neg,
																										dplyr::select(rosmap_sig_genes_biomart_hits, hgnc_symbol, entrezgene),
																										by = c("gene_symbol" = "hgnc_symbol"))

# How many genes are missing entrez ID hits?
length(which(is.na(rosmap_signif_reagan_pos_v_neg1$entrezgene)))

# How many unique entrez IDs are there in the hits?
length(unique(rosmap_signif_reagan_pos_v_neg1$entrezgene))
```

Check whether the 'ProbeID's in the original rosmap data are entrez or ensembl IDS
```{r}
rosmap_data$ProbeID[grep("FZD9", rosmap_data$Symbol)]
rosmap_data$ProbeID[grep("A2ML1", rosmap_data$Symbol)]
rosmap_data$TargetID[grep("ACOX1", rosmap_data$Symbol)]
```
These (and a few others we tried) don't match to the entrez IDs from Biomart for those gene sybmols, so we'll have to stick with our original approach.  

Create network ID column
```{r}
rosmap_signif_reagan_pos_v_neg1$kegg_id <- sapply(rosmap_signif_reagan_pos_v_neg1$entrezgene, function(x) {
	ifelse(is.na(x), 
				 NA,
				 paste("hsa", x, sep = ":"))
})				 
...
## Query the KEGG database with the network names

```{r, message = F}
# create vector of kegg IDs
kegg_ids <- na.omit(unique(rosmap_signif_reagan_pos_v_neg1$kegg_id))

library(KEGGREST)
test <- keggGet(kegg_ids)
```
```{r}
biocLite("Homo.sapiens")
library(Homo.sapiens)
...
```{r}
rosmap_gtypes <- read.table("~/Dropbox/eQTL/Data/ROSMAP_Data/ROSMAP_arrayGenotype.bed", header = F, nrow = 5, sep = "\t", na.strings = "NA")
```
