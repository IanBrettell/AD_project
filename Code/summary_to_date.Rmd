...
```
Shows that there are `r length(setdiff(signif_all$probe_id, pid_all$probe_id))` probe ids in 'signif_all' that are not in 'pid_all', i.e. they do not have any annotation data.
...
```
`r length(setdiff(rownames(exdata), pid_symb$probe_id))`/`r nrow(exdata)` probes are not accounted for in the annotation data?
...
meta <- read.table("~/Dropbox/eQTL/Coding/AD_project/Working/key_metadata.txt", header = T) # read in metadata
...
```{r}
# read in table
snp_gns <- read.delim("~/Dropbox/eQTL/Coding/AD_project/Working/aibl_snp_genes.txt", header = T)
...
```{r}
pk_ass <- read.table("~/Dropbox/eQTL/Coding/AD_project/Working/plink/plink.assoc", na.strings = NA, header = T, fill = T)
pk_fsh <- read.table("~/Dropbox/eQTL/Coding/AD_project/Working/plink/plink.assoc.fisher", na.strings = NA, header = T, fill = T)
...
snp_gns <- read.delim("~/Dropbox/eQTL/Coding/AD_project/Working/aibl_snp_genes.txt", sep = "\t", header = T, stringsAsFactors = F)
library(dplyr)
...
```{r}
probes <- read.delim("~/Dropbox/eQTL/Coding/AD_project/Working/affy_probes_annotated.txt", header = T, sep = "\t")
