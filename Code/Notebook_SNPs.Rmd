...
# unique SNPs and the genes they are in
unique(sig_eqtl_snps$snps)
length(unique(sig_eqtl_snps$snps))
unique(as.character(sig_eqtl_snps$hgnc_symbol))
length(unique(as.character(sig_eqtl_snps$hgnc_symbol)))

# unique differentially expressed genes
unique(sig_eqtl_gns$gene_symbol)
length(unique(sig_eqtl_gns$gene_symbol))

# find any hits with the genes implicated in the 2018 study
sig_eqtl_snps[grep("PICALM|BIN1|CR1|ABCA7|MS4A6A|CLU|FERMT2|DSG2|EPHA1|SORL1|ZCWPW1", sig_eqtl_snps$hgnc_symbol), ]
sig_eqtl_gns[grep("PICALM|BIN1|CR1|ABCA7|MS4A6A|CLU|FERMT2|DSG2|EPHA1|SORL1|ZCWPW1", sig_eqtl_gns$hgnc_symbol), ]

```
17 significant AIBL SNPs in 16 genes (17 less the NA), affecting the expression levels of 11 genes (12 less the NA).
...
# unique SNPs and the genes they are in
unique(pos_sig_eqtl_snps$snps)
length(unique(pos_sig_eqtl_snps$snps))
unique(as.character(pos_sig_eqtl_snps$hgnc_symbol))
length(unique(as.character(pos_sig_eqtl_snps$hgnc_symbol)))

# unique differentially expressed genes
unique(pos_sig_eqtl_gns$gene_symbol)
length(unique(pos_sig_eqtl_gns$gene_symbol))

# find any hits with the genes implicated in the 2018 study
pos_sig_eqtl_snps[grep("PICALM|BIN1|CR1|ABCA7|MS4A6A|CLU|FERMT2|DSG2|EPHA1|SORL1|ZCWPW1", pos_sig_eqtl_snps$hgnc_symbol), ]
pos_sig_eqtl_gns[grep("PICALM|BIN1|CR1|ABCA7|MS4A6A|CLU|FERMT2|DSG2|EPHA1|SORL1|ZCWPW1", pos_sig_eqtl_gns$hgnc_symbol), ]
...
```
32 SNPs in 29 unique genes (30 less NA) affecting 32 unique genes (33 less NA). None of those genes are shared. The SNPs get a hit with *EPHA1*. 
...
# find any hits with the genes implicated in the 2018 study
neg_sig_eqtl_snps[grep("PICALM|BIN1|CR1|ABCA7|MS4A6A|CLU|FERMT2|DSG2|EPHA1|SORL1|ZCWPW1", neg_sig_eqtl_snps$hgnc_symbol), ]
neg_sig_eqtl_gns[grep("PICALM|BIN1|CR1|ABCA7|MS4A6A|CLU|FERMT2|DSG2|EPHA1|SORL1|ZCWPW1", neg_sig_eqtl_gns$hgnc_symbol), ]
...
```
27 SNPs in 32 unique genes affecting the expression of 33 unique genes. None of those genes are shared. The SNPs get a hit with *ABCA7*, *CLU*, and *EPHA1*.
...
```
So all differentially expressed genes in each group are unique to that group other than *EPHA1*. 
