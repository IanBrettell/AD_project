...
j <- lapply(kegids, function(x){ # pull hsa ids for genes that have an exact match
  if (length(x) > 1 && length(names(x)[grep(paste("\\b", x, "\\b", sep = ""), x)]) == 1){
    print(names(x))
  }
})

lapply(kegids, function(x){
  length(names(x[grep(paste("\\b", x, "\\b", sep = ""), x)])) > 1
})

k <- data.frame(unlist(j))# convert into data frame
k$gene <- rownames(k)
colnames(k)[1] <- "hsa_id"
l <- merge(h, k, by = "gene", all.x = T, sort = F)
...
lapply(m, function(x){
  if ((length(x) > 1) && (length(names(x[grep(paste("\\b", x, "\\b", sep = ""), x)])) == 1)){
    print(names(x[grep(paste("\\b", x, "\\b", sep = ""), x)]))
  }
})
